...
export default function ProductListPage() { // Renamed for clarity
    ...
    const handleDeleteProduct = (productId) => {
        ...
        //     }
        // }
        console.log(`Delete product ${productId} - to be implemented with confirmation`);
    };
    ...
    const getStockTag = (stock) => {
        if (stock === 0) {
            ...
            return <Badge className="bg-yellow-100 text-yellow-800 hover:bg-yellow-100">庫存不足</Badge>
        } else { // Removed redundant stock >= 10 check
            return <Badge className="bg-blue-100 text-blue-800 hover:bg-blue-100">庫存充足</Badge>
        }
    }
    ...
    const columns = useMemo(() => [
        ...
        },
        {
            ...
            cell: ({ row }) => <div className="text-sm text-gray-600">{row.original.updatedAt}</div>,
        },
        {
            id: 'actions',
            header: '管理',
    ...
    ], [router]); // Added router to dependencies if handleEditProduct is memoized with it
    ...
    return (
        <div className="container mx-auto py-8 px-4">
                ...
                <CardContent>
                    ...
                    <div className="flex items-center justify-between mt-6">
                        <div className="text-sm text-gray-700">
                            顯示 {table.getRowModel().rows.length > 0 ? table.getState().pagination.pageIndex * table.getState().pagination.pageSize + 1 : 0} 到{' '}
            ...
            {selectedProductForDialog && (
                <Dialog open={!!selectedProductForDialog} onOpenChange={() => setSelectedProductForDialog(null)}>
                    <DialogContent className="max-w-lg">
                        ...
                        <div className="space-y-4 mt-4">
                            <div className="flex justify-center">
                                <Image src={selectedProductForDialog.image}
                            ...
                            <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                                <div><span className="font-medium">品牌：</span> {selectedProductForDialog.brand}</div>
                                <div><span className="font-medium">分類：</span> {selectedProductForDialog.category}</div>
                                <div><span className="font-medium">價格：</span> NT$ {selectedProductForDialog.price.toLocaleString()}</div>
                                <div><span className="font-medium">庫存：</span> {selectedProductForDialog.stock} 件 {getStockTag(selectedProductForDialog.stock)}</div>
                                <div className="col-span-2"><span className="font-medium">狀態：</span> {getStatusTag(selectedProductForDialog.status)}</div>
                                <div className="col-span-2">
                                    <span className="font-medium">描述：</span>
                                    <p className="mt-1 text-gray-600 whitespace-pre-wrap">{selectedProductForDialog.description || '無描述資訊'}</p>
                                </div>
                                <div className="col-span-2 text-xs text-gray-500">
                                    <span className="font-medium">最後更新：</span> {selectedProductForDialog.updatedAt}
                                </div>
                            </div>
                        </div>
                    </DialogContent>
