...
router.get('/member', async (req, res) => {
  ...
  try {
    const [coupons] = await db.query(
      `
      SELECT coupons.*, coupons_user.state_id
      FROM coupons_user
      JOIN coupons ON coupons_user.coupon_id = coupons.id
      WHERE coupons_user.user_id = ? AND coupons_user.state_id = ?
      `,
      [user_id, state_id]
    )
...
router.post('/member', async (req, res) => {
  ...
  try {
    await db.execute(
      'INSERT INTO coupons_member (user_id, brand_id, category_id, skin_type_id, min_amount, amount, valid_from, valid_to, description) VALUES (?, ?, ?, ?, ?, ?, NOW(), ?, ?)',
      [
        user_id,
        brand_id,
        category_id,
