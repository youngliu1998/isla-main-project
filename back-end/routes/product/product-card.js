...
// POST /api/products/:productId/favorite - 切換收藏（模擬示例）
router.post('/api/products/:productId/favorite', (req, res) => {
  const { productId } = req.params
  const product = products.find((p) => p.productId === productId)
  ...
  console.log(`用戶操作：切換商品 ${productId} 的收藏狀態 (模擬)`)
  res.json({ message: `商品 ${productId} 收藏狀態已切換 (模擬)` })
})

// POST /api/cart - 加入購物車（模擬示例）
router.post('/api/cart', (req, res) => {
  ...
  console.log(`用戶操作：將 ${quantity} 件商品 ${productId} 加入購物車 (模擬)`)
  res.json({
    message: `${quantity} 件 ${product.productName} 已加入購物車 (模擬)`,
  })
})
